...
func renderISAByID(ctx context.Context, st storage.DoltStorage, id string) (string, error) {
	if st == nil {
		return "", errors.New("renderISAByID: nil store")
	}
	accessor, ok := storage.UnwrapStore(st).(storage.RawDBAccessor)
	if !ok {
		return "", errors.New("renderISAByID: store does not expose raw DB access")
	}
	db := accessor.DB()
	if db == nil {
		return "", errors.New("renderISAByID: store DB is nil")
	}
...
func renderISAByIDWithDB(ctx context.Context, db *sql.DB, id string) (string, error) {
	if db == nil {
		return "", errors.New("renderISAByIDWithDB: nil db")
	}
	row, err := loadRenderRow(ctx, db, id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return "", fmt.Errorf("isa not found: %s", id)
		}
		var wk *wrongKindError
		if errors.As(err, &wk) {
			return "", fmt.Errorf("%s is not an ISA (kind=%s)", wk.id, wk.kind)
		}
		return "", fmt.Errorf("reading isa: %w", err)
	}
