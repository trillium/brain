...
-- Partial-migration guard (R1): the 0053 pre-repair can add the split-target
-- columns before this file runs, which makes @needs_migrate false while the
-- legacy key/indexes/constraints below are still in place. Detect leftover
-- legacy state by the old index name (absent from the post-migration index
-- set) so the transformation below still completes instead of being skipped.
SET @needs_legacy_cleanup = (
    SELECT IF(COUNT(*) > 0, 1, 0)
    FROM INFORMATION_SCHEMA.STATISTICS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'wisp_dependencies'
      AND INDEX_NAME = 'idx_wisp_dep_depends'
);
...
SET @sql = IF((@needs_migrate = 1 OR @needs_legacy_cleanup = 1),
